...
<body>
    <div class="container">
        <h2>Reserve Table for Customer</h2>
        
        <form method="POST" action="admin_reserve_table.php">
            ...
            <label for="customer_phoneno">Customer Phone:</label>
        ...
        <div class="back-link">
            <a href="admin_dashboard.php">Redirect to Dashboard</a>
        </div>
    </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<script>alert('Table reserved successfully!'); window.location.href='admin_dashboard.php';</script>";
        } else {
            // New customer, insert into customer table
            $sql = "INSERT INTO customer (name, email, phoneno) VALUES (?, ?, ?)"; // Make sure to match the field name
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sss", $customer_name, $customer_email, $customer_phone); // Use $customer_phone
            $stmt->execute();
            ...
            echo "<script>alert('Table reserved successfully for new customer!'); window.location.href='admin_dashboard.php';</script>";
        }
